...
#if DOUX_USE_FLOAT64
    EXPECT_NEAR(grad[1], (v2-v1)/(2*eps), 1E-7 * std::abs(grad[1]));
#else
    EXPECT_NEAR(grad[1], (v2-v1)/(2*eps), 1E-3 * std::abs(grad[1]));
#endif
  }
...
#if DOUX_USE_FLOAT64
    EXPECT_NEAR(grad[5], (v2-v1)/(2*eps), 1E-7 * std::abs(grad[5]));
#else
    EXPECT_NEAR(grad[5], (v2-v1)/(2*eps), 1E-3 * std::abs(grad[5]));
#endif
  }
...
#if DOUX_USE_FLOAT64
    EXPECT_NEAR(grad[6], (v2-v1)/(2*eps), 1E-7 * std::abs(grad[6]));
#else
    EXPECT_NEAR(grad[6], (v2-v1)/(2*eps), 1E-3 * std::abs(grad[6]));
#endif
  }
}